...
interface DocumentFile {
  ...
  document_type: string;
  namespace: string;
  user_id: string; // This now contains the full URL path
}
...
const PreviousDocuments: React.FC<PreviousDocumentsProps> = ({
  ...
  // Get the file name from either user_id or bucket_path
  const getFileName = (record: DocumentFile): string => {
    // First try to extract from user_id if it contains a full path
    if (record.user_id && record.user_id.includes("/")) {
      const parts = record.user_id.split("/");
      return parts[parts.length - 1];
    }

    // Fall back to bucket_path
    if (record.bucket_path && record.bucket_path.includes("/")) {
      const parts = record.bucket_path.split("/");
      return parts[parts.length - 1];
    }

    // If no slashes, just return the bucket_path as is
    return record.bucket_path || "Unknown";
  };

  // Get the proper URL for viewing/downloading
  const getFileUrl = (record: DocumentFile): string => {
    // If user_id has a full URL with https://, use that
    if (record.user_id && record.user_id.startsWith("http")) {
      return record.user_id;
    }

    // Otherwise, we need to construct the URL properly
    if (record.user_id && !record.user_id.startsWith("http")) {
      return `https://storage.googleapis.com/buildrfi-bucket/${record.user_id}`;
    }

    // Last resort, try bucket_path
